...
export const SignUp: React.FC<SignUpProps> = ({ title, formId }) => {
  ...
  const { data, isLoading } = useGetFormQuery(formId);
	...
	React.useEffect(() => {
		console.log(cookies);
		getRegistration({ id: formId, email: cookies[`protos-form-${formId}`]?.email });	
	}, [formId])
	...
	React.useEffect(() => {
		if (registrationData.data?.data) {
			...
			setInitialValues({ ...formValues, email: cookie?.email, firstName: cookie?.firstName, lastName: cookie?.lastName });
		}
	}, [registrationData])
  ...
  async function submit(
    ...
    try {
      await triggerRegistration({ id: formId, data: values, anonymous: isAnonymous });
      setSubmitting(false);
      ...
      log.debug("Succesfully registered for activity");
			...
			if (isAnonymous) {
				setCookie(`protos-form-${formId}`, { email: values.email, firstName: values.firstName, lastName: values.lastName });	
			}
    } catch (e) {
      log.error(e);
    }
