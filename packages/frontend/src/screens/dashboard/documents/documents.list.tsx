...
export const DocumentsList: React.FC = () => {
  ...
  React.useEffect(() => {
    console.log("revalidator", revalidator.state);
  }, [revalidator.state]);
  ...
  async function categorySubmit(
    ...
    try {
      await updateCategoryApi({
        id: data.category.id,
        body: { name: values.name, type: values.type as CategoryType },
      }).unwrap();
              ...
              <CardHeader
                action={
                  <Checkbox
                    ...
                    onClick={() => updateSelected(file.id)}
                  />
                }
                sx={{ padding: 0, position: "absolute" }}
              />
              <CardMedia
                src={`${import.meta.env.VITE_APP_OBJECT_STORAGE_URL}/${file.path}`}
                component="img"
                height="194"
              />
            </Card>
      ...
      <Card>
        <CardContent>
          <CategoryForm
            ...
            title={t("documents:update-category")}
          />
          ...
          <Stack direction="row" gap={2} marginY={3}>
            ...
            <Box marginLeft="auto">
              ...
              >
                {" "}
                {selected.length} {t("documents:files")}
              </Button>
            </Box>
          </Stack>
      ...
      <Dialog open={visible} onClose={() => setVisible(false)}>
        ...
        <DialogActions>
          <DialogActions>
            ...
            <Button variant="contained" onClick={() => removeFiles()}>
              {t("common:remove")}
            </Button>
          </DialogActions>
        </DialogActions>
