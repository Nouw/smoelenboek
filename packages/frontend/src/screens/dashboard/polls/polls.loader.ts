...
export const pollsLoader: ActionFunction = async () => {
  ...
  const req = store.dispatch(propollApiSlice.endpoints.getPolls.initiate());
...
export const pollLoader: ActionFunction = async ({ params }) => {
  await rehydrationPromise;

  const req = store.dispatch(
    propollApiSlice.endpoints.getPoll.initiate(+params.id!),
  );
