...
export class EmailOutboxProcessor implements OnModuleInit, OnModuleDestroy {
  ...
  private trigger(): void { void this.drain().catch((error) => this.logger.error(JSON.stringify({ event: 'email.outbox_sweep_failed', error: errorMessage(error) }))); }
  async drain(): Promise<void> {
    ...
    try {
      for (const message of await this.outbox.claim()) {
        ...
        this.logger.warn(JSON.stringify({ event: 'email.delivery_failed', messageId: message.id, messageType: message.messageType, attempts: message.attempts + 1 }));
        }
      }
