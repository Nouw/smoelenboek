...
export default class TeamController {
  ...
  async getTeam(@Request() req, @Response() res) {
    ...
    const players = await Database.createQueryBuilder(
      UserTeamSeason,
      "userTeam",
    )
      .select([
  ...
  @Guard(["CREATE_TEAM"])
  @Post("/", [
    body(["name", "rank"]).exists(),
    body("gender").exists().customSanitizer(TeamController.typeSanitizer),
  ])
  ...
  async updatePlayer(@Request() req, @Response() res) {
    ...
    const userTeamSeason = await Database.manager.findOneBy(UserTeamSeason, {
      id,
    });
